...
DROP PROCEDURE P_ADD_HISTORY
GO

DROP FUNCTION GET_ACTUAL_VACANCY
DROP FUNCTION GET_COUNT_HISTORY_FOR_APPLICANT
GO
...
CREATE TABLE SPECIALITY(
	...
	COUNT_EMPLOYEE INT NOT NULL DEFAULT 0,
...
CREATE TRIGGER AIUD_EMPLOYEE ON EMPLOYEE
AFTER INSERT,UPDATE,DELETE
AS
BEGIN
	UPDATE SPECIALITY SET COUNT_EMPLOYEE = COUNT_EMPLOYEE + (select COUNT(*) From  inserted i WHERE i.ID_SPECIALITY = SPECIALITY.ID)
		WHERE ID IN (select distinct(ID_SPECIALITY) from inserted)
	UPDATE SPECIALITY SET COUNT_EMPLOYEE = COUNT_EMPLOYEE - (select COUNT(*) From  deleted D WHERE D.ID_SPECIALITY = SPECIALITY.ID)
		WHERE ID IN (select distinct(ID_SPECIALITY) from deleted)
END 
GO
...
AS
BEGIN
	DELETE FROM V_HISTORY WHERE ID = @ID
END
GO
...
AS
BEGIN
	UPDATE  HISTORY SET [STATUS] = @STATUS
		WHERE ID = @ID
END
GO                                                                                   

CREATE PROCEDURE P_ADD_HISTORY ( @ID_APPLICANT INT, @ID_VACANCY INT)
AS
BEGIN
	IF(EXISTS(SELECT * FROM HISTORY WHERE STATUS =0 AND ID_VACANCY = @ID_VACANCY AND ID_APPLICANT=@ID_APPLICANT))
	BEGIN
		RAISERROR('Не больше одной вакансии', 16 ,1)
		RETURN
	END
	INSERT INTO HISTORY (ID_EMPLOYEE,ID_VACANCY,ID_APPLICANT,DATE_ADD)
		VALUES(1,@ID_VACANCY,@ID_APPLICANT, SYSDATETIME());
END
GO 
...
------------------- Создание функций ---------------------------------------------------------------------------------------------------------------------------
----- возможные вакансии для соискателя( подбор по врофессиям )
CREATE FUNCTION GET_ACTUAL_VACANCY(@APPLICANT_ID INT)
...
AS
BEGIN
	DECLARE CUR CURSOR  FOR SELECT ID_VACANCY FROM HISTORY WHERE ID_APPLICANT = @APPLICANT_ID AND  STATUS = 0
	INSERT INTO @RES 
		SELECT  ID, ID_ORGANIZATION, ID_PROFESSION, ABOUT ,STATUS, MIN_EXPERIENCE ,MAX_EXPERIENCE, DATE_ADD, NAME_ORGANIZATION, NAME_PROFESSION 
		FROM V_VACANCY 
		WHERE ID_PROFESSION IN (SELECT ID_PROFESSION FROM  PROFESSION_SKILL WHERE ID_APPLICANT = @APPLICANT_ID)
	
	DECLARE @H_V_ID INT;
	OPEN CUR
	FETCH CUR INTO @H_V_ID
	WHILE @@FETCH_STATUS = 0
	BEGIN
		DELETE FROM @RES WHERE ID = @H_V_ID
		FETCH CUR INTO @H_V_ID
	END
	CLOSE CUR
	DEALLOCATE CUR
...
---- количествоисторий для соискателя
CREATE FUNCTION GET_COUNT_HISTORY_FOR_APPLICANT(@APPLICANT_ID INT)
RETURNS INT
AS 
BEGIN
	DECLARE @RES INT
	SELECT @RES = COUNT(*) FROM HISTORY WHERE ID_APPLICANT=@APPLICANT_ID
	RETURN @RES
END